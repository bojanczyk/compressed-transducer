...
\begin{itemize}
\item  Often, one needs to not only store compressed large data, but also to 
process and analyse it as well. 
...
algorithms. One example, which arises in group theory, is the compressed word problem:  checking if two compressed representations describe the same group element, see the surveys~\cite{BKLMNRS20}  and~\cite{Loh14}.
\end{itemize}
We begin our investigation of algorithmics on compressed data by claryfing two aspects: (i)
...
a unique production with left-hand side $Y$.
  An equivalent representation is a \emph{straight line program}, which is a sequence of instructions that assign values to variables, using concatenation and constants, as in the following example, which generates the string $aaaa$:
\begin{lstlisting}
    x := 'a' 
    y := x $\cdot$ x
    z := y $\cdot$ y
\end{lstlisting}
The variables can be seen as non-terminals, and the variable $z$ in the last line can be seen as the starting non-terminal. A straight line  program can achieve exponential compression, but not more: if the program has $n$ lines then it can generate a string of length up to $2^n$.
...
We now come to the second question (ii), which is about the form of compression that we use. 
