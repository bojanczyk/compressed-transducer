...
Characterizing the class of all string-to-string functions that are compatible with compression is a hopeless task. In this paper, we are interested in  string-to-string functions that can be computed by finite-state transducer models. As explained in Example~\ref{ex:exp-output}, the transducers cannot have exponential size outputs, which excludes many important transducer models, such as copyful streaming string transducers \cite[Section 2.1]{filiotCopyfulStreamingString2017} or the even  more general model of  macro tree transducers (in the string-to-string case) \cite[Section 3.1]{DBLP:journals/jcss/EngelfrietV85}.  For this reason, we will only consider transducer models that produce outputs of polynomial size. The largest known class of string-to-string transducers with polynomial size outputs are the \emph{polyregular functions} \cite{polyregular-survey}, and thus all transducers in this paper will be special cases of the polyregular functions. We will consider three such classes: 
\begin{align*}
...
\end{definition}
Since the marked squaring function is polyregular, it follows that the rectangular polyregular functions are a subclass of the polyregular functions. By design, this subclass is compatible with compression, as stated in the following theorem, which is proved using the same argument as for regular functions in Theorem~\ref{thm:regular-compatible}.
...
\paragraph*{A normal form.} We conclude this section with a slightly different characterization of rectangular polyregular functions, which relies only on composition in the closure properties and does not use the map combinator. 
...
\begin{example}[Duplication, squaring and rectangle] \label{ex:squaring-rectangle-duplication} Let us begin with a program in a Haskell-like language that implements the duplication function from Example~\ref{ex:duplication} (we use blue superscripts to indicate the types of  variables and  auxiliary functions):
    \begin{align*}
    \lambda \church x {\blue{A^*}}.\ \church \concatterm {\blue{A^{**} \to A^*}} [x,x].
    \end{align*}
Here, the function $\concatterm$ concatenates a list of lists into a single list. Next, we write a program that implements
    ...
    \begin{align*}
    \lambda x^{\blue{A^*}}.\ \concatterm^{\blue{A^{**} \to A^*}} (\mapterm^{\blue{(A \to A^*) \to A^* \to A^{**}}} \ (\lambda y^{\blue{A}}.\ x)\ x)
    \end{align*}
The subprogram $\mapterm \ (\lambda y : A.\ x)\ x$ applies the function $\lambda y.x$ to each list element of $x$, which means that each letter in the list $x$ will be replaced by the list $x$ itself. For example, if the input to this subprogram is $[1,2,3]$, then its output will be the nested list $[[1,2,3],[1,2,3],[1,2,3]]$. The nesting is then removed by the $\concatterm$ function. 
Using a similar program, we can implement a variant of the rectangle function from Example~\ref{ex:strength}, in which the input is two strings
\begin{equation*}
    \lambda x^{\blue{A^*}}.\  \lambda y^{\blue{A^*}}. \  \concatterm^{\blue{A^{**} \to A^*}} ( \mapterm^{\blue{(A \to A^*) \to A^* \to A^{**}}} \ ( \lambda z^{\blue{A}} . \ \concatterm^{\blue{A^{**} \to A^*}} [[z],y] ) \ x )  
  \end{equation*}
  \qed
\end{example}
...
% \), where $M_{i_1},\ldots,M_{i_k}$ are the terms of type $A$ in the input list, and $N_j$ is the list of all terms of type $B$ between $M_{i_j}$ and $M_{i_{j+1}}$, with the convention that $i_0$ represents the beginning of the input, and $i_{k+1}$ represents its end.
    ...
    \end{figure}
        ...
        \paragraph*{Reduction} So far, we have explained how a program can be written as a string over a finite alphabet. We now explain how programs can be evaluated under this representation. 
