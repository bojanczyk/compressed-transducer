...
\author{Mikołaj Bojańczyk}{University of Warsaw, Poland \and \url{https://www.mimuw.edu.pl/~bojan/}}{bojan@mimuw.edu.pl}{https://orcid.org/0000-0002-7758-1072}{Mikołaj Bojańczyk was supported by the Polish National Science Centre (NCN) grant ``Polynomial finite state computation'' (2022/46/A/ST6/00072).}
...
two-way transducers. Finally, polyregular functions are computed by deterministic two-way transducers with pebbles. The regular and polyregular
...
\begin{tikzpicture}
 ...
 \node [above left = .2cm and .4cm of l1, align = left]  (t1) {\small if the run begins in this  initial \\ \small state then  output $b$};
 \node [above right = .2cm and .4cm of l2, align = left]  (t2) {\small if the run ends in this  final \\ \small state then  output $b$};
 ...
 inner (\blue{blue}) nonterminals  and symbols from $A$:
   ...
   Observe that the partition, if it exists, is unique: the inner nonterminals are those which cannot derive a string containing the separator symbol (they correspond to parts of list items), and the outer ones are those which do (they correspond to sequences of list items).
    ...
    \begin{claim}
        ...
        |}$ can be converted in polynomial time into an equivalent compression which is in normal form.
        \end{claim}
    \begin{proof}
...
to reverse). Equivalence of this definition with \cref{def:polyregular-def} can be shown in the same way as \cref{lem:map-normal-form} below.}.
...
\begin{proof}
    ...
    any compression of $w_n$ must have size at least $2^n$. The reason is that $w_n$ contains an infix of the form  $ba^i\underline a$ for every $i \in \set{1,\ldots,{2^n}}$. On the other hand, the number of such infixes is limited by the size of any compression, as shown in the following claim
...
\end{theorem}
Since the rectangle function is polyregular, it follows that the rectangular polyregular functions are a subclass of the polyregular functions. 
...
\end{proof}
By \cite[Theorem 6.1]{NguyenNP21}, the class of comparison-free polyregular functions contains the class of regular functions. Hence, we also have the inclusion
regular $\subsetneq$ rectangular polyregular. 
...
1w2w 
        \qquad \stackrel{\text{rational}}\mapsto \quad   ww.
        \end{align*}
        ...
        \end{align*}
        From this we obtain the correct output by removing every comma or $|$ that  does not directly follow a $\#$ 
        and finally removing every $\#$.  This is a regular function.
...
\begin{equation} \label{prog for rectangle}
 ...
 The outermost application of $\concatterm$ in \eqref{prog for rectangle} flattens  this nested list into $[a,1,2,3,b,1,2,3]$.
  \qed
\end{example}
...
write down a program as a string, and to then evaluate it by using string rewriting. If done carefully, the evaluation can be done by a rectangular polyregular function.
...
% program for a rational function of type $A^* \to B^*$ and $M$ is of type $A^*$ and in normal form then $R \, M$ is in normal form. \todo{I added this sentence; is it correct?} 
  ...
  \subparagraph{A reduction strategy.} The rest of this proof follows the same lines as the one in~\cite[Theorem 4.1]{polyregular-survey}. Define a \emph{redex} in a program to be a node in its syntax tree to which a reduction rule can be applied. A redex can also be seen as an infix of the string representation of the program. Here is an example of a program with some (in fact, all) redexes underlined:
    ...
    \end{corollary}
    \begin{proof}
    Fix the $k$ and the parallel reduction strategy from  \cref{lem:reduction-strategy}; these 
 only depend on the resource bound and not on the input program.
...
$k$ repetitions of the parallel reduction strategy yield a normal form, even if thereby the height of the syntax tree increases.}. 
\end{enumerate}
...
\begin{theorem}
    Let $A$ and $B$ be finite alphabets. A string-to-string function
    \begin{align*}
    f : A^* \to B^*
    \end{align*}
    ...
    \begin{enumerate}
        ...
        \item \label{item:atomic-strength} contains, for all arrow-free types $A$ and $B$, the strength function explained in~\eqref{eq:strength}.
...
\begin{enumerate}
    ...
    \item Can the rectangular polyregular functions be characterised in terms of logic? Such a characterisation exists for the  general polyregular functions; these  are exactly the \mso interpretations~\cite[Theorem 7]{msoInterpretations}. For the rectangular ones a promising idea is to consider the interpretations based on \cite[Conjecture 10.1]{NguyenNP21}:
Starting with a word $w$ represented in the usual way as a structure, one first takes a $k$-fold product 
of this structure with itself, and then applies a $1$-dimensional MSO-interpretation that produces the output word.
Conjecture 10.1 from~\cite{NguyenNP21}
 was originally made for the weaker class of comparison-free polyregular functions, but it has been refuted~\cite[Theorem 4.6]{kiefer2023refutations}. However, it might still be appropriate for the rectangular polyregular functions.
    \item  Can the rectangular polyregular functions be characterised in terms of an  imperative programming language, such as the for-transducers from~\cite[Section 1]{polyregular-survey}?
\end{enumerate}
