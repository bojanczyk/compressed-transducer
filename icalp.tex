...
% for repeating theorems, using restatable
%\usepackage{thmtools}
...
\begin{proof}
 ...
 inner (\blue{blue}) nonterminals  and symbols from $A$:
   ...
   \begin{center}    
        \begin{tikzpicture}
	\node[rectangle, fill = light-gray]  (a) at (0,0) {$\phantom{|}\! a\,a\,a\,a\,b\,a\,b\,b\,a\,a\,b\,a\,a\,b$};
	\node[anchor=west, rectangle, fill = light-gray, right = 1.5mm of a.east]  (b) {$|\,a\,b\,a\,b\,a\,b\,b\,|\,a\,b\,a\,b\,a\,b\,b\,a\,a$};
        \node[anchor=west,right = 0mm of b.east] (c) {$|$};
        \node[anchor=west, rectangle, fill = light-gray, right = 0mm of c.east] (d) {$\phantom{|}\! a\,a\,b\,a\,a\,b\,a\,a\,a\,b\,b\,a\,a\,b\,a\,b\,b$};
        \node[anchor=north west, align=left] at (a.south west) {\small \textbf{start} \\ before first separator, \\ not including it};
        \node[anchor=north west, align=left] at (b.south west) {\small \textbf{middle} \\ between first and last \\ separators, including the \\ first but not the last one.};
        \node[anchor=north west, align=left] at (d.south west) {\small \textbf{finish} \\ after last separator, \\ not including it};
   \end{tikzpicture}
   \end{center}
...
write down a program as a string, and to then evaluate it be using string rewriting. If done carefully, the evaluation can be done by a rectangular polyregular function.
 ...
 \node [right = 1cm of r]  (t) {\small application};
 \path  (t) edge[->,gray] (r) ;
\end{tikzpicture}
\end{center}
%    \mypic{2}
...
\begin{enumerate}
    ...
    \item Can the rectangular polyregular functions be characterised in terms of logic? Such a characterisation exists for the  general polyregular functions; these  are exactly the \mso interpretations~\cite[Theorem 7]{msoInterpretations}. For the rectangular ones a promising idea is to consider the interpretations described in \cite[Conjecture 10.1]{NguyenNP21}:
Starting with a word $w$ represented in the usual way as a structure $\mathcal{A}_w$, one first takes a $k$-fold product 
$\mathcal{A}_w^k$ and then applies a $1$-dimensional MSO-interpretations that produces the output word.\todo{\cite[Conjecture 10.1]{NguyenNP21}  
 refers to 1-dim. FO-interpretations and FO-comparision-free polyregular functions and the authors write that you mentioned that the
naive transposition of their conjecture to MSO fails. Is your idea to take 1-dim. MSO-interpretations applied to $k$-fold powers for 
 rectangular polyregular functions?} 
Conjecture 10.1 from~\cite{NguyenNP21}
 was originally made for the weaker class of comparison-free polyregular functions, but it has been refuted~\cite[Theorem 4.6]{kiefer2023refutations}. However, it might still be appropriate for the rectangular polyregular functions.
