...
Using a similar program, we can implement a variant of the rectangle function from Example~\ref{ex:strength}, in which the input consists of two strings
\begin{equation*}
...
% \), where $M_{i_1},\ldots,M_{i_k}$ are the terms of type $A$ in the input list, and $N_j$ is the list of all terms of type $B$ between $M_{i_j}$ and $M_{i_{j+1}}$, with the convention that $i_0$ represents the beginning of the input, and $i_{k+1}$ represents its end.
  ...
  \subsection{Reduction} So far, we have explained how a program can be written as a string over a finite alphabet. We now explain how programs can be evaluated under this representation. 
    ...
    \end{align}
The complete list of reduction rules is in Figure~\ref{fig:reduction-rules}.
...
% program for a rational function of type $A^* \to B^*$ and $M$ is of type $A^*$ and in normal form then $R \, M$ is in normal form. \todo{I added this sentence; is it correct?} 
   ...
   \subsection{A reduction strategy} The rest of this proof follows the same lines as the one in~\cite[Theorem 4.1]{polyregular-survey}. Define a \emph{redex} in a program to be a node in its syntax tree to which a reduction rule can be applied. A redex can also be seen as an infix of the string representation of the program. Here is an example of a program with some (in fact, all) redexes underlined:
    ...
    \end{corollary}
    \begin{proof}
        By Lemma~\ref{lem:reduction-strategy}, there is some $k$, which only depends on the resource bound and not on the input program, such that the program can be reduced to normal form by repeating the following process $k$ times: (1) mark the redexes using a rational function; and (2) reduce the marked redexes. Since the rectangular polyregular functions are closed under composition and contain the rational ones, it remains to prove that part (2) of the process can be implemented using a rectangular polyregular function.
