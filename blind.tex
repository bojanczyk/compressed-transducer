...
We think that the rectangular polyregular functions are interesting in their own right, and not just some random subclass of the polyregular functions that  is compatible with compression. In particular, it seems plausible that the converse implication of Theorem~\ref{thm:polyregular-compatible} also holds: if a polyregular function is compatible with compression, then it is rectangular polyregular. \todo{Is there some evidence supporting this conjecture?} \mb{I was thinking about it for a few hours, and it seems a plausible enough to conjecture, there are even some possible lines of attack.} Proving this conjecture seems difficult, and we leave it for future work.  
...
\begin{enumerate}
    ...
    \end{align*}
    In the data constructors $\leftterm$ (resp., $\rightterm$), the other type $B$ (resp., $A$) is part of the syntax, since it cannot be inferred from the argument $M$. \todo[inline]{Does this mean that, strictly speaking, we should write $\rightterm_A M$ and $\leftterm_B M$? I do not say that we should do this, but as a remark this might make the point clearer.}
    \mb{Yes, that is correct.}
    ...
    \end{align*}
    \todo[inline]{Should the previous line be: 
                $\blockterm : (A+B)^* \to A^* \times (B \times A^*)^*$ ?}
    \mb{Yes, you are correct, thanks, fixed it!}
...
\begin{proof}
    ...
    $ \lambda w.\ M\  (N\  w).$
    Also, they are closed under map and reverse, \todo{I cannot find reverse in the syntax of the polyregular $\lambda$-calculus.} 
    \mb{Thanks, fixed this omission!} since both of these are included as atomic programs. Duplication and rectangle were treated in Example~\ref{ex:squaring-rectangle-duplication}.
...
\todo[inline]{I think, concerning the height of a program on should  be a bit more formal. At the end it should be a simple inductive definition.
Is the following correct?
 height of single variable $x$ or an atomic program = 0. \\
 height of $MN$ (application) = 1 + height of $M$ + height of $N$ \\  (similarly for $(M,N)$ and $[M_1, \ldots, M_n]$). \\
 height of $\lambda x. M$ = 1 + height of $M$ (similarly for $\rightterm M$ and $\leftterm M$).}
 \mb{Oh, I see that it was unclear, because the definition you spell out is not the one that I had in mind. For me, the height is the maximum distance from the root to a leaf, so for example in application it will be 1 + max( height of M, height of N), and similarly for pairs and lists.} 
