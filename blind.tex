...
\mb{I made the statement softer, since I do not know how to write a few  lines of supporting evidence that would not be completely incoherent}
...
\begin{enumerate}
    ...
    \end{align*}
    In the rule list, the program $[]$ is also allowed, representing the empty list. Observe that in the data constructors for co-pairs,  the other type $B$ (resp, $A$) is part of the syntax, since it cannot be inferred from the argument $M$. 
...
It is easy to see that applying a reduction rule changes neither the type nor the semantics of a program.  A program is in \emph{normal form} if no reduction rules can be applied to it. The normal form is unique, up to renaming of bound variables~\cite[Corollary 1.4.9]{sorensenLecturesCurryHowardIsomorphism2006}. If a closed program represents a string, i.e.~its type is $A^*$ for some finite type, then its normal form is necessarily a list of elements of $A$. Therefore, for closed programs that have such a type, normal forms and their string representations are essentially the same thing.
\mb{I removed the comment about normal form for rational functions, which was incorrect. I added the appropriate reduction rule to the figure.} 
...
% program for a rational function of type $A^* \to B^*$ and $M$ is of type $A^*$ and in normal form then $R \, M$ is in normal form. \todo{I added this sentence; is it correct?} 
    ...
    \begin{proof}
        ...
        [M\ N_1, \ldots, M\ N_n].
        \end{align*}
        This reduction rule can be evaluated similarly to the proof of Lemma~\ref{lem:substitution}, by  using the rectangle function to copy the program $M$ into each of the $n$ list elements. The reduction rule for reverse can be implemented by a regular function, and the remaining reduction rules are even rational. \mb{Rewrote this to explain your concerns. Block is rational (it is just a rearranging of parentheses).  Reverse indeed is not rational but regular.  Split is not even regular, but it is not allowed. }
